...
import { useRegisterLearnerMutation } from "../../../features/auth/authApi";
...
const LearnerRegistration = () => {
  ...
  console.log("Email from learner registration", email);
  // React hook form
  const {
    register,
    handleSubmit,
  ...
  const [registerLearner, learnerUpdateStates] = useRegisterLearnerMutation();
  ...
  useEffect(() => {
    if (!isLoading && email && learnerUpdateStates?.isSuccess) {
  ...
  }, [isLoading, email, navigate, reset, from, learnerUpdateStates?.isSuccess]);
